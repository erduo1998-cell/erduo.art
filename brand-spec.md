...
- 叙事顺序：个人介绍 → 四项服务入口 → 会员与实际业务交付 → 经历 → 作品与产品 → 知识资产 → 公开工具 → 联系。
...
- `business.css` / `business.js` 扩展首页业务展示和原生咨询弹窗，保留现有 Q 版人物与动效。
- 会员：199 元永久、首批 100 席，79 人 / 剩余 21 席是 2026-09-08 的人工确认快照，需同时维护 HTML 数值、meter 与日期。不是实时库存。
- 线下课：南京 3 天，5980 元 / 人；具体班期咨询确认。AI 结果陪跑：9800 元 / 月，每周 4 次一对一、每次 2 小时，目标与安排开始前确认。
- 企业合作分为定制搭建移交、模板开发与后续内容生产，范围与报价按项目确定。
- 会员预览采用真实手册封面/首张操作页；完整资料不在公共站点提供下载。当前四份实物为 20 / 16 / 59 / 30 页；页数是手册规模，不代表软件版本。
- 微信二维码复用 `assets/qrcode.jpg`，它是个人加好友码，按钮按服务呈现备注；不标为直接入群码或支付入口。
- 所有原有作品、开源链接及 `/reachsurge/`、`/knowledge/` 产品子页保留。导航分别提供 PRODUCTS、OPEN SOURCE 入口。
